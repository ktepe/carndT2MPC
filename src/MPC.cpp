...
#if ket_debug
		cout << "in FG Eval " << endl;
		cout << "size of fg " << fg.size() << endl;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // 4 * 10 + 2 * 9
#if ket_debug
		cout << "in mpc::solve " << endl;
		cout << "size of state " << state.size() << endl;
		cout << "size of coeffs " << coeffs.size() << endl;
#endif
  ...
  for (int i = 0; i < N-2; i++) {
    ...
    controls.push_back(solution.x[y_start + i + 1]);
  }
#if ket_debug
	cout << "controls " << controls.size() << endl ;
#endif
  return controls;
